...
public class DeleteNetwork {
    ...
    public static Map<String, String> execute(@Param(value = InputNames.TOKEN, required = true) String token,
                                              @Param(value = InputNames.NETWORK_ID, required = true) String networkId,
        ...
        try {
            ...
            requestWrapper.setHost(StringUtils.toRequiredString(networkId, ""));
            ...
            requestWrapper.setRequestMethod("DELETE");
            requestWrapper.setRequestUri("/v2.0/networks/" + networkId);

            resultMap = httpRequestService.deleteNetwork(requestWrapper, token);
        } catch (Exception ex) {
